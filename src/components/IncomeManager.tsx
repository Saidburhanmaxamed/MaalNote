...
import { formatCurrency, formatDate, INCOME_CATEGORIES, PAYMENT_METHODS } from '../utils';
...
export default function IncomeManager({
  ...
  const totalFilteredIncome = filteredIncomes.reduce((acc, tx) => acc + tx.amount, 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Manage your store inflows, sales revenues, and services.
          </p>
        </div>

        <button
          onClick={openAddModal}
          className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-semibold text-sm transition-colors shadow-lg shadow-emerald-600/10 cursor-pointer self-start sm:self-auto"
        >
      ...
      <div className="p-6 rounded-2xl bg-emerald-50/50 dark:bg-emerald-950/20 border border-emerald-100/60 dark:border-emerald-900/30 flex items-center justify-between">
        <div>
          <span className="text-xs font-bold text-emerald-700 dark:text-emerald-400 uppercase tracking-wider">Total Filtered Inflows</span>
          <h2 className="text-3xl font-extrabold text-emerald-800 dark:text-emerald-300 mt-1">
            {formatCurrency(totalFilteredIncome, user.currency)}
          </h2>
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-1 font-semibold">
            Across {filteredIncomes.length} recorded entries
          </p>
        </div>
        <div className="p-4 rounded-xl bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 shrink-0">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4 rounded-xl bg-white dark:bg-slate-900/40 border border-slate-100 dark:border-slate-800 backdrop-blur-sm shadow-sm">
        ...
        <div className="relative flex-1">
          ...
          <input
            type="text"
            placeholder="Search income by details, category, payment method..."
            value={searchQuery}
      ...
      <div className="p-6 rounded-2xl bg-white dark:bg-slate-900/40 border border-slate-100 dark:border-slate-800 backdrop-blur-sm shadow-sm overflow-x-auto">
        {filteredIncomes.length > 0 ? (
              ...
              {filteredIncomes.map((tx) => (
                <tr key={tx.id} className="hover:bg-slate-50/55 dark:hover:bg-slate-850/30 transition-colors">
                  <td className="py-3 px-4">
                    <div className="text-slate-900 dark:text-white font-bold">{tx.description || 'General Income'}</div>
                  </td>
                  <td className="py-3 px-4">
                    <span className="inline-flex px-2 py-0.5 rounded-full text-[10px] font-bold text-emerald-600 bg-emerald-500/10">
                      {tx.category}
                    </span>
                  </td>
          ...
          <div className="flex flex-col items-center justify-center p-12 text-slate-400 text-center">
            <DollarSign className="w-12 h-12 mb-3 opacity-40 text-emerald-500" />
            <h3 className="font-bold text-slate-900 dark:text-white text-base">No income logs found</h3>
            <p className="text-xs max-w-sm mt-1">
              Adjust your search query, change filters, or record a new income ledger line to begin tracking.
            </p>
          </div>
        )}
      </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              ...
              <h3 className="font-extrabold text-slate-900 dark:text-white text-lg tracking-tight">
                {editingTransaction ? 'Edit Income Transaction' : 'Record New Income Inflow'}
              </h3>
              <p className="text-xs text-slate-500 mt-1">
                Ensure exact records for perfect monthly financial alignment.
              </p>
              ...
              <form onSubmit={handleSubmit} className="mt-6 space-y-4">
                ...
                <div>
                  ...
                  <div className="relative">
                    <div className="absolute left-3.5 top-1/2 -translate-y-1/2 text-emerald-500 font-bold text-sm">$</div>
                    <input
                      type="number"
                      required
                      min="0.01"
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <textarea
                      ...
                      placeholder="e.g., Cash sales for inventory model X..."
                      rows={2}
